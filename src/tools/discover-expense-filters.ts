  ...
  _meta: {
    openai: {
      toolInvocation: {
        ...
        invoked:
          "Filter values discovered. Use suggested_filter in get_expenses.",
      },
    },
  },
};
...
export const schema = {
  ...
  dimensions: z
    .array(
      z.object({
        dimension_type: z
          .enum(["standard", "tag"])
          .describe("Type of dimension"),
        name: z
          .string()
          .describe(
            "Dimension name (e.g., 'service_name', 'region_id') or tag key"
          ),
        search: z
          .string()
          .optional()
          .describe("Optional search term to filter values"),
        limit: z
          .number()
          .default(100)
          .describe("Max values to return per dimension"),
      })
    )
  ...
  auto_select: z
    .boolean()
    .default(true)
    .describe("Whether to auto-select discovered values in suggested_filter"),
};

export default async function discover_expense_filters(
  params: InferSchema<typeof schema>
) {
  const { organization_id, dimensions, auto_select = true } = params;

  const discovered: Record<string, unknown> = {};
  const suggested_filter: Record<string, unknown> = {};
    ...
    for (const dim of dimensions) {
      const dimensionParam =
        dim.dimension_type === "standard"
          ? {
              dimension_type: "standard" as const,
              name: dim.name as
                | "service_name"
                | "service_category"
                | "resource_type"
          ...
          : {
              dimension_type: "tag" as const,
              tag_type: "values" as const,
              key: dim.name,
            };
