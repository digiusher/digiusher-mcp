...
const resourceTagFilterSchema = z
  .object({
    ...
    exists: z
      ...
      .optional()
      .describe("Filter by tag key existence"),
  })
...
export const schema = {
  organization_id: z
    .string()
    .uuid()
    .describe("The organization ID to query recommendations for"),
  scenario_id: z
    .union([z.array(z.string()), z.null()])
    .optional()
  ...
  scenario_tags: z
    ...
    .optional()
    .describe(
      "Filter by scenario tags (e.g., 'quick_win', 'idle', 'orphaned')"
    ),
  status: z
    .union([z.array(recommendationStatusEnum), z.null()])
    .optional()
    .describe(
      "Filter by recommendation status (e.g., 'open', 'applied', 'dismissed')"
    ),
  severity: z
  ...
  sort_by: z
    .union([sortByEnum, z.null()])
    .optional()
    .describe("Field to sort results by (default: annual_savings)"),
  sort_direction: z
    .union([sortDirectionEnum, z.null()])
    .optional()
...
// Tool implementation
export default async function list_recommendations(
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/v3/organizations/${organization_id}/recommendations`,
      {
        method: "POST",
